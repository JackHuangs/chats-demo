...
var kefu = {
	version:1.2, 	//当前kefu.js的版本
	api:{
		domain:'https://api.kefu.leimingyun.com',				//domain域名，设置如 https://xxxxxxx.com   前面要带协议 ，后面要不带 /
	...
	//当前kefu.js操作的document对象
	document:{
		iframeId:'', //iframe的id,如果客服在iframe中显示，那这里是显示客服界面的iframe 的 id
		//获取当前kefu.js操作的document对象
		get:function(){
			if(kefu.document.iframeId.length < 1){
				//不在iframe显示，那在当前页面显示
				return document;
			}else{
				return document.getElementById(kefu.document.iframeId).contentWindow.document;
			}
		},
		//设置当前kefu.js操作的document对象。如果不设置，默认操作的是当前页面，如果设置，传入iframe元素id，则kefu显示在iframe中，避免被当前页面的css影响
		set:function(iframe_id){
			kefu.document.iframeId = iframe_id;
		}
	},
	//初始化，当kefu.js 加载完毕后，可以执行这个，进行im的初始化
	init:function(){
		var head0 = kefu.document.get().getElementsByTagName('head')[0];
		...
		for(var key in kefu.extend){
			...
			//加载模块的css. 如果在iframe中，css也要加载到iframe中
			if(kefu.extend[key].css != null && kefu.extend[key].css.length > 0){
				var link = kefu.document.get().createElement('link');
				link.type='text/css';
				link.rel = 'stylesheet';
		...
		list:{
			...
			render:function(){
				if(kefu.ui.list.renderAreaId.length > 0){
					//有设置渲染区域，那么渲染到设置的id上
					if(kefu.document.get().getElementById(kefu.ui.list.renderAreaId) != null){
						kefu.document.get().getElementById(kefu.ui.list.renderAreaId).innerHTML = kefu.ui.list.html;
					}
				}else{
					kefu.document.get().body.innerHTML = kefu.ui.list.html;
				}
				
				if(kefu.ui.list.listItemTemplate.length < 1){
					//设置某个用户聊天item的模板
					kefu.ui.list.listItemTemplate = kefu.document.get().getElementById('chatlist').innerHTML;   
				}
			    ...
			    kefu.document.get().getElementById('chatlist').innerHTML = html;
		...
		chat:{
			...
			html:`
				<div id="mobile">
					...
					<div id="newMessageRemind">
						...
						<div id="newMessageRemindClose" onclick="kefu.document.get().getElementById('newMessageRemind').style.display='none';">X</div>
					</div>
				    ...
				    <footer id="chat_footer">
				        <div id="input_area">
				            <div id="textInput">
				                ...
				                <input type="submit" value="发送" class="send" id="sendButton" onclick="kefu.chat.sendButtonClick();" style="background-color:{shuruTypeColor};">
				            </div>
				            <div id="inputExtend">
				                <!-- 其他，如图片、商品、订单 -->
			...
			appendMessage: function(message){
			    ...
			    if(message.type == 'SYSTEM'){
			        ...
			        kefu.document.get().getElementById('chatcontent').appendChild(section);
			        //滚动条滚动到最底部
			...
			generateMessageSection:function(message){
			    ...
			    //发送文本消息后绘制对话窗口
			    var section = kefu.document.get().createElement("section");
			...
			scrollToBottom:function(){
				//console.log('height:'+kefu.document.get().getElementById('chatcontent').scrollHeight);
				kefu.document.get().getElementById('chatcontent').scrollTo(0,kefu.document.get().getElementById('chatcontent').scrollHeight);
			},
			//在当前chat一对一聊天界面的消息最末尾追加显示一条系统消息, text:要显示的消息内容
			appendSystemMessage:function(text){
				chatcontent = kefu.document.get().getElementById('chatcontent');
			...
			newMessageRemind:function(message){
				kefu.cache.getUser(message.sendId, function(user){
					var remindTextDiv = kefu.document.get().getElementById('newMessageRemindText');
					...
					//显示这条消息
					kefu.document.get().getElementById('newMessageRemind').style.display = 'block';
				});
			},
			//文字输入框被点击，隐藏扩展功能区域,已废弃，有 kefu.chat.switchToJianpanShuruType() 代替
			textInputClick:function (){
				//隐藏扩展功能输入区域
//				kefu.document.get().getElementById('inputExtend').style.display = '';
//				kefu.document.get().getElementById('inputExtendShowArea').style.display = 'none';
...
//				}
			...
			render:function(otherUserId){
				kefu.ui.chat.html = kefu.ui.chat.html.replace(/{shuruTypeColor}/g,kefu.ui.color.shuruTypeColor); //颜色替换，发送按钮颜色遵循 kefu.ui.color.shuruTypeColor 的设置。 v1.2 增加
				if(kefu.ui.list.renderAreaId.length > 0){
					//有设置渲染区域，那么渲染到设置的id上
					if(kefu.document.get().getElementById(kefu.ui.chat.renderAreaId) != null){
						kefu.document.get().getElementById(kefu.ui.chat.renderAreaId).innerHTML = kefu.ui.chat.html;
					}
				}else{
					//赋予chat聊天窗html的大框信息显示
					kefu.document.get().body.innerHTML = kefu.ui.chat.html;
				}
			    ...
			    kefu.chat.getOtherUser(otherUserId, function(data){
			        ...
			        //设置网页聊天窗title 的对方昵称
					kefu.document.get().getElementById('nickname').innerHTML = kefu.chat.otherUser.nickname;
					//对方在线状态
			        kefu.document.get().getElementById('onlineState').innerHTML = data.onlineState;
			        if(kefu.document.get().getElementById('otherUserHead') != null){
			        	//聊天窗口中对方用户的头像
			        	kefu.document.get().getElementById('otherUserHead').src = kefu.getImageUrl(data.user.head);
			        }
			        
			        //将对方用户发言的头像换为接口拉取的真实头像。如果当前chat模板中显示头像的话
			        try{
			        	var heads = kefu.document.get().getElementsByClassName("otherUser");
...
//				    							var auto = kefu.document.get().getElementsByClassName('AUTO_REPLY')[0];
//				    							if(typeof(auto) != 'undefined'){
//				    								auto.parentElement.removeChild(auto);
//				    							}
//				    							
//				    							//绘制同步过来的消息
//				    							var chatcontent = kefu.document.get().getElementById('chatcontent');
...
//			            		}
			        ...
			        kefu.document.get().getElementById('chatcontent').onscroll = function(){
			            //console.log(document.getElementById('chatcontent').scrollTop);
			            if(kefu.document.get().getElementById('chatcontent').scrollTop < 900){
			                //还剩一页半了，那么提前开始加载上一页
			...
			pc:{
				...
				//关闭chat窗口
				close:function(){
					kefu.document.get().getElementById('pc').parentNode.removeChild(kefu.document.get().getElementById('pc'));
					kefu.ui.list.entry();
				},
				...
				init:function(){
					...
					kefu.extend.pc={
						initChat:function(){
							...
							kefu.document.get().getElementById('close').innerHTML = kefu.ui.images.close.replace(/{color}/g,kefu.ui.color.extendIconColor);
						}
					};
...
//				},
				...
				sizeChange:{
					...
					moveInit:function(){
						var chatWindows = kefu.document.get().getElementById('pc');
						...
						var contentWindows = kefu.document.get().getElementById('chatcontent');
						...
						//缩放效果
						var father = kefu.document.get().body;
						var scale = kefu.document.get().getElementById('footerButton');
					    scale.onmousedown = function (e) {
					      ...
					      father.onmousemove = function (ev) {
					        ...
					        if(kefu.ui.chat.pc.sizeChange.currentY > 0){
					        	...
					        	kefu.document.get().getElementById('chatcontent').style.height = (height-kefu.ui.chat.pc.sizeChange.height_chat_content_size)+'px';
					        }else{
				...
				//鼠标点击移动、放大缩小的初始化
				moveInit:function(){
					kefu.ui.chat.pc.chatWindowDiv = kefu.document.get().getElementById('pc');
					kefu.ui.chat.pc.chatHeadWindowDiv = kefu.document.get().getElementById('head');
...
//			    }
//			}
//			kefu.document.get().getElementById('inputExtend').innerHTML = inputExtendHtml;
			
			
			//设置底部的输入方式切换
			if(kefu.document.get().getElementById('shuruType') != null){
		...
		loadHistoryList(){
			if(!kefu.chat.currentLoadHistoryList){
				...
				request.post(kefu.api.get(kefu.api.chatLog),{token:kefu.token.get(),otherId:kefu.chat.otherUser.id, time:kefu.chat.chatMessageStartTime, type:'before'}, function(data){
					...
					var chatcontent = kefu.document.get().getElementById('chatcontent');
					//删除聊天记录加载中的提示
					chatcontent.removeChild(kefu.document.get().getElementById('historyListLoading'));
		...
		sendButtonClick:function (){
		    var value = kefu.document.get().getElementById('text').innerHTML;
		    if(value.length == 0){
		        msg.info('尚未输入');
		        return;
		    }
		    ...
		    kefu.chat.sendTextMessage(kefu.document.get().getElementById('text').innerHTML);
			msg.close();	//关闭发送中提示
		    //清空内容区域
		    kefu.document.get().getElementById('text').innerHTML = '';
		...
		shuruTypeChange:function(){
			...
			if(kefu.chat.shuruType == 'jianpan'){
				//当前是键盘输入，切换到更多输入方式
				kefu.chat.shuruType = 'more';
				kefu.document.get().getElementById('shuruType').innerHTML = kefu.ui.images.jianpan.replace(/{color}/g,kefu.ui.color.shuruTypeColor);
				...
				kefu.document.get().getElementById('inputExtendShowArea').style.display = '';
			}else{
				//当前是更多输入，切换到键盘输入方式
				kefu.chat.shuruType = 'jianpan';
				kefu.document.get().getElementById('shuruType').innerHTML = kefu.ui.images.more.replace(/{color}/g,kefu.ui.color.shuruTypeColor);;
				
				//清空掉插件内容显示的区域，如表情插件显示出来的表情选择所在的区域，将表情列表清空掉
				kefu.document.get().getElementById('inputExtendShowArea').innerHTML = '';
				...
				kefu.document.get().getElementById('inputExtend').innerHTML = '<div class="extendSmallIcon">'+inputExtendHtml+'</div>';
			}
		},
		//切换到键盘输入类型
	...
	extend:{
		...
		face:{
			...
			onclick:function (){
				...
				//隐藏扩展功能栏
				kefu.document.get().getElementById('inputExtend').style.display = 'none';
				kefu.document.get().getElementById('inputExtendShowArea').style.display = '';

				kefu.document.get().getElementById('inputExtendShowArea').innerHTML = html;
			...
			/* 向输入框中插入表情 */
			insert:function (key){
				kefu.document.get().getElementById('text').innerHTML = kefu.document.get().getElementById('text').innerHTML + kefu.extend.face.faces[key];
			}
		...
		image:{
			...
			onclick:function(){
				//添加input改动监听
				if(kefu.document.get().getElementById('imageInput').oninput == null){
					kefu.document.get().getElementById('imageInput').oninput = function(e){
					    if(typeof(e.srcElement.files[0]) != 'undefined'){
					        ...
					        kefu.document.get().getElementById('imageInput').value = '';
					    }    
					}
				}

				kefu.document.get().getElementById('imageInput').click();
			},
			//放大全屏查看图片
		...
		luyin : {
		    ...
		    onclick:function(){
				if(window.location.protocol == 'http:'){
					msg.info('发送语音功能无法使用。<br/>因为当前网页的协议是http协议，只有在https协议下才能使用此功能。');
					return;
				}
				try{
		...
		file : {
		    ...
		    onclick:function(){
		    	var input = kefu.document.get().createElement("input");
		    	input.type = "file";
		    	input.click();
